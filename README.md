...
Machine-voice audio A/B: append `?voice=reed|liturgy|presence|choir|vox|hum` to any
URL (default is the shipped grain; `vox`/`hum` are the voice-like ones). Switch live
from devtools with `__voice('vox')`.
